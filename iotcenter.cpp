...
//Libraries I have manually added
#include "uop_msb.h"
...
//Sensor variables
AnalogIn lightReal(PC_0);
uop_msb::EnvSensor sensors;

extern EnvDataBuffer leENVDataBufferNetwork;

//Sends sensor data to Azure server when interrupt fires
void SendDataToAzureServer() {
    //printf("NetworkData ISR started\n\r");
    ...
    char message[120]; //originally 80
    for (int i = 0; i < 10; ++i) {
        if (message_received) {
        ...
        */
        ...
        //Acquires latest values from network data buffer
        EnvData latestValues;
        latestValues = leENVDataBufferNetwork.Pop();
        double LightLevel = latestValues.light;
        double Temperature = latestValues.temperature;
        double Pressure = latestValues.pressure;

        /*double light = (float) lightReal;
        double temp = (float) sensors.getTemperature();
        double pressure = (float) sensors.getPressure();*/

        sprintf(message, "{ \"LightLevel\" : %5.2f, \"Temperature\" : %5.2f, \"Pressure\" : %5.2f }", LightLevel, Temperature, Pressure);
